...
def movement(object, directions, speed, x, y):
...
def movement1(object, directions, speed):
    if directions == 1:
        object.position[1] += speed
        object.frame = 0
    elif directions == 2:
...
def testYourLuck(user, screen):
    from random import randint
    if user.item != 0 and user.weapon != 0:
        user.item = 0
        user.weapon = 0
        user.lvl += randint(0, user.lvl)
        #text = readHistory("view/data_base/text/testYourLuckYes.txt")
        return user.lvl
    else:
        #text = readHistory("view/data_base/text/testYourLuckNo.txt")
        return None
    return None